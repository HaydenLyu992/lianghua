...
| **行业 & 概念板块分析** | 基于全市场多维数据，AI 识别当前最热行业板块和概念板块，推荐主板标的 |
| **涨跌停专项分析** | 涨停/跌停池、封板强度、连板高度、炸板率 |
| **资金流向排行榜** | 实时主力资金净流入/流出 TOP 50 + 飙升榜 |
| **趋势排行** | 基于日涨跌 + 5日K线涨跌 + 主力资金 + 涨跌停信号的多维趋势评分，AI 趋势解读 |
| **在线回测** | 自定义策略 + 历史数据回测，输出收益曲线和绩效指标 |
| **LLM 深度分析** | 8 人辩论制综合分析 + 因子深度解读 + 新闻情感分类 + 板块轮动分析（DeepSeek 强制启用）|
...
将所有最新数据发送给 LLM，生成：
1. **当前最热行业板块 TOP5** — 含热力指数、核心驱动逻辑（政策/资金/情绪）、持续性判断、推荐主板标的
2. **当前最热概念板块 TOP5** — 含概念催化剂、与行业板块共振情况、持续性判断、推荐主板标的
3. **未来 1-4 周潜力板块 TOP3** — 含催化剂事件、信号强度、潜在标的
4. **风险警示** — 需回避板块 + 市场风险判断

> 推荐个股自动过滤创业板(300/301)、科创板(688)、北交所(8)，仅推荐主板标的。

## 趋势排行

基于多维度量化的趋势评分模型，全市场扫描识别上升/下跌趋势个股：

**评分因子：**
- 当日涨跌幅（权重 ×3，钳位 ±10%）
- 5日K线涨跌幅（权重 ×1.5，钳位 ±30%）
- 主力资金净流入（权重 ×2，钳位 ±5亿）
- 涨跌停信号（涨停 +30，跌停 -30）

**趋势强度分级：** 强势(≥60) → 走强(30-60) → 偏强(10-30) → 震荡(-10~10) → 偏弱(-30~-10) → 走弱(-60~-30) → 弱势(<-60)

支持一键触发 LLM 趋势解读，分析当前市场整体趋势特征、领涨/领跌板块共性、资金面持续性判断及风险提示。
